...
export default function WhatsAppFab({ defaultText }: { defaultText?: string }) {
  const waNumber = process.env.NEXT_PUBLIC_WHATSAPP ?? "+57300XXXXXXX";
  const phone = waNumber.replace(/[^0-9+]/g, "");
  const defaultMsg = defaultText
    ? defaultText
    : "Hola, quisiera información sobre sus servicios y catálogo.";
  ...
  return (
    <a
      ...
      className="fixed right-4 bottom-6 z-50 inline-flex items-center gap-3 rounded-full bg-[#25D366] px-4 py-3 text-sm font-medium text-black shadow-lg md:right-8 md:bottom-8"
    >
      ...
      <span className="hidden sm:inline">Chatea por WhatsApp</span>
    </a>
  );
}
